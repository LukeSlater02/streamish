    ...
    public class VideoRepository : BaseRepository, IVideoRepository
    {
        ...
        public Video GetById(int id)
        {
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
 SELECT Title, Description, Url, Video.DateCreated as VidDate, UserProfileId,
 UserProfile.Name as UserName, UserProfile.Email as UserEmail, UserProfile.ImageUrl, UserProfile.DateCreated as UserDate
                            FROM Video join UserProfile on video.UserProfileId = UserProfile.Id
                           WHERE Video.Id = @id";
        ...
        public Video GetVideoByIdWithComments(int id)
        {
            ...
            using var cmd = conn.CreateCommand();
            {
                cmd.CommandText = @"SELECT v.id VidId, Title, Description, Url, v.DateCreated VidDate,
                                        v.UserProfileId CreatorId,
                                        u.Name UserName, Email, ImageUrl, u.DateCreated UserDate,
                                        c.Id CommentId, Message, VideoId, c.UserProfileId CommenterId FROM Video v
                                        JOIN UserProfile u on v.UserProfileId = u.Id LEFT JOIN Comment c 
                                        ON c.VideoId = v.Id 
                                        WHERE v.Id = @id";
                DbUtils.AddParameter(cmd, "@id", id);
